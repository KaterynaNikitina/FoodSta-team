...
let bordrWidth = Number(getComputedStyle(document.querySelector(".dishes__wrapper"), null).getPropertyValue("border-width").slice(0, -2));
let timer;
let offset = 0;
let stepSlider = document.querySelector(".dishes__img").clientWidth + bordrWidth;
let positionSlider = (countImg - 1) * stepSlider;
let screenSize = document.documentElement.clientWidth;
// Auto slider
if (screenSize > 767) autoSlider();
function autoSlider() {
...
function sliderLeft() {
    offset += stepSlider;
    if (offset > positionSlider) offset = 0;
    clearTimeout(timer);
...
function sliderRigth() {
    ...
    if (offset < 0) offset = positionSlider;
    clearTimeout(timer);
    autoSlider();
    sliderLine.style.left = -offset + "px";
    // Reset focus after click button
...
window.addEventListener("resize", function() {
    ...
    if (screenSize > 767) autoSlider();
    setTimeout(function() {
        bordrWidth = Number(getComputedStyle(document.querySelector(".dishes__wrapper"), null).getPropertyValue("border-width").slice(0, -2));
